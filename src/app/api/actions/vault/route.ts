...
export const POST = async (req: Request) => {
	try {
		...
		const { action, amount, errorMessage } = validateQueryParams(new URL(req.url));
		if (errorMessage) {
			return Response.json(
...
function validateQueryParams(requestURL: URL): { action: string, amount: number, errorMessage?: string } {
	// Validate the action and amount parameters
